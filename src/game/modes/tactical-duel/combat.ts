...
const buildCombatEvents = (
  ...
  destinationOccupant: CombatResolution["destinationOccupant"],
): readonly GameEventPayload[] => [
  {
    type: "COMBAT_RESOLVED",
    attackerUnitId: originalAttacker.id,
    defenderUnitId: originalDefender.id,
    attackerStance: originalAttacker.stance,
    defenderStance: originalDefender.stance,
    attackerAttack: originalAttacker.card.baseAttack,
    defenderAttack: originalDefender.card.baseAttack,
    attackerDefenseBefore: originalAttacker.currentDefense,
    attackerDefenseAfter: nextAttacker.currentDefense,
    defenderDefenseBefore: originalDefender.currentDefense,
    defenderDefenseAfter: nextDefender.currentDefense,
    attackerStatusAfter: nextAttacker.status,
    defenderStatusAfter: nextDefender.status,
    attackerMovedToDestination: destinationOccupant === "attacker",
    outcome,
  },
  ...
  return {
    ...
    value: {
      ...
      events: buildCombatEvents(
        ...
        resolution.defender,
        resolution.outcome,
        resolution.destinationOccupant,
      ),
    },
  };
};
