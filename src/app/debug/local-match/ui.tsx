...
import type { MatchPlayerState, PlayerMatchView, UnitView } from "@/game";
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {reserves.length === 0 ? (
          ...
          reserves.map((unit) => (
            ...
            >
              <span className="font-bold">{unit.revealed ? unit.card.cardName : "伏せカード"}</span>
    ...
    return (
      <section className="rounded-3xl border border-slate-700/80 bg-slate-900/80 p-4">
        ...
        <p className="mt-3 text-sm leading-6 text-slate-400">
          盤面またはリザーバーのカードをタップすると、安全な表示モデルに含まれる範囲で詳細を表示します。
        </p>
      </section>
    );
  }
...
export default function LocalMatchDebugClient({
  ...
  const [selectedUnitId, setSelectedUnitId] = useState<UnitView["unitId"] | null>(
    view.units.find((unit) => unit.status === "board")?.unitId ?? null,
  );
      ...
      <div className="mx-auto flex w-full max-w-md flex-col gap-4 px-3 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(1rem,env(safe-area-inset-top))]">
        ...
        <section className="rounded-3xl border border-slate-800 bg-slate-900/80 p-3">
          ...
          <p className="mt-3 text-xs leading-5 text-slate-400">
            現在の視点: {viewerSide}。内部座標は変更せず、表示行・列の順序だけを視点に合わせて切り替えます。
          </p>
        </section>
              ...
              row.cells.map((cell) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => setSelectedUnitId(unit?.unitId ?? null)}
                    className={`aspect-square min-w-0 rounded-xl border p-1 text-[0.55rem] transition focus:outline-none focus:ring-2 focus:ring-cyan-300 ${
                      selected
          ...
          {view.players.map((player) => (
            <div key={player.id} className="grid gap-3">
              <div className="rounded-2xl border border-slate-800 bg-slate-900/70 px-3 py-2 text-xs text-slate-400">
                {formatPlayerLabel(player)} / viewer side:{" "}
                {getPlayerSide(view, player.id)}
              </div>
